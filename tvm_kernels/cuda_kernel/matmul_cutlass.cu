    ...
    if (Version == 0){
      ...
      if (length >= 128){
        if (dim_Y_out >= 128){
          cudaEventRecord(start);
          for (int i=0; i < 100; i++){
            verify_matmul_fuse_fn<cutlass::gemm::GemmShape<128,128,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
              length, dim_in, dim_Y_out, dim_out,
              x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d,
              x_lw_d, x_uw_d, y_lw_d, y_uw_d);
          }
        } else if (dim_Y_out >= 64){
          cudaEventRecord(start);
          for (int i=0; i < 100; i++){
            verify_matmul_fuse_fn<cutlass::gemm::GemmShape<128,64,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
              length, dim_in, dim_Y_out, dim_out,
              x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d,
              x_lw_d, x_uw_d, y_lw_d, y_uw_d);
          }
          ...
          cudaEventRecord(start);
          for (int i=0; i < 100; i++){
            verify_matmul_fuse_fn<cutlass::gemm::GemmShape<128,32,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
              length, dim_in, dim_Y_out, dim_out,
              x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d,
              x_lw_d, x_uw_d, y_lw_d, y_uw_d);
          }
        ...
        if (dim_Y_out >= 128){
          ...
          for (int i=0; i < 100; i++){
            verify_matmul_b<<<length * dim_Y_out, 32>>>(x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d, length, dim_out, dim_Y_out);
            verify_matmul_fn<cutlass::gemm::GemmShape<128,128,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
                length * dim_in, dim_Y_out, dim_out, 
                x_lw_d, x_uw_d, w_d, y_lw_d, y_uw_d);
            // verify_matmul_w_gemm<128, 128, 32, 64, 32, 16, 16, 8, 3><<<grid_dim, block_dim, maxbytes>>>(x_lw_d, x_uw_d, w_d, y_lw_d, y_uw_d, length, dim_in, dim_out, dim_Y_out);
          }
        } else if (dim_Y_out >= 64){
          cudaEventRecord(start);
          for (int i=0; i < 100; i++){
            verify_matmul_b<<<length * dim_Y_out, 32>>>(x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d, length, dim_out, dim_Y_out);
            verify_matmul_fn<cutlass::gemm::GemmShape<128,64,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
              length * dim_in, dim_Y_out, dim_out, 
              x_lw_d, x_uw_d, w_d, y_lw_d, y_uw_d);
          }
        } else {
          ...
          for (int i=0; i < 100; i++){
            verify_matmul_b<<<length * dim_Y_out, 32>>>(x_lb_d, x_ub_d, w_d, y_lb_d, y_ub_d, length, dim_out, dim_Y_out);
            verify_matmul_fn<cutlass::gemm::GemmShape<128,32,32>, cutlass::gemm::GemmShape<64, 32, 32>, 3>(
              length * dim_in, dim_Y_out, dim_out, 
              x_lw_d, x_uw_d, w_d, y_lw_d, y_uw_d);
          }
        }
...
int main(int argc, char **argv){
    // verify_matmul<0>(128, 128, 128, 128);
    // verify_matmul<1>(128, 128, 128, 128);
    // verify_matmul<2>(128, 128, 128, 128);

    // verify_matmul<0>(128, 64, 64, 64);
    // verify_matmul<1>(128, 64, 64, 64);
    // verify_matmul<2>(128, 64, 64, 64);

    // verify_matmul<0>(64, 64, 64, 64);
    // verify_matmul<1>(64, 64, 64, 64);
    // verify_matmul<2>(64, 64, 64, 64);

    // verify_matmul<0>(64, 128, 128, 128);
    // verify_matmul<1>(64, 128, 128, 128);
    // verify_matmul<2>(64, 128, 128, 128);

    // printf("==============\n");

    // verify_matmul<0>(64, 256, 256, 256);
    // verify_matmul<1>(64, 256, 256, 256);
    // verify_matmul<2>(64, 256, 256, 256);

    // printf("==============\n");

    // verify_matmul<0>(64, 512, 512, 512);
    // verify_matmul<1>(64, 512, 512, 512);
    // verify_matmul<2>(64, 512, 512, 512);
    
    // printf("==============\n");

    // verify_matmul<0>(64, 1024, 1024, 1024);
    // verify_matmul<1>(64, 1024, 1024, 1024);
    // verify_matmul<2>(64, 1024, 1024, 1024);

    // printf("==============\n");

    // verify_matmul<0>(32, 64, 64, 64);
    // verify_matmul<1>(32, 64, 64, 64);
    // verify_matmul<2>(32, 64, 64, 64);

    // printf("==============\n");

    // verify_matmul<0>(32, 128, 128, 128);
    // verify_matmul<1>(32, 128, 128, 128);
    // verify_matmul<2>(32, 128, 128, 128);

    // printf("==============\n");

    // verify_matmul<0>(32, 256, 256, 256);
    // verify_matmul<1>(32, 256, 256, 256);
    // verify_matmul<2>(32, 256, 256, 256);

    // printf("==============\n");

    // verify_matmul<0>(32, 512, 512, 512);
    // verify_matmul<1>(32, 512, 512, 512);
    // verify_matmul<2>(32, 512, 512, 512);

    // printf("==============\n");

    // verify_matmul<0>(32, 1024, 1024, 1024);
    // verify_matmul<1>(32, 1024, 1024, 1024);
    // verify_matmul<2>(32, 1024, 1024, 1024);

    // printf("==============\n");

    // verify_matmul<0>(16, 64, 64, 64);
    // verify_matmul<1>(16, 64, 64, 64);
    // verify_matmul<2>(16, 64, 64, 64);

    // printf("==============\n");

    // verify_matmul<0>(16, 128, 128, 128);
    // verify_matmul<1>(16, 128, 128, 128);
    // verify_matmul<2>(16, 128, 128, 128);

    // printf("==============\n");

    // verify_matmul<0>(16, 256, 256, 256);
    // verify_matmul<1>(16, 256, 256, 256);
    // verify_matmul<2>(16, 256, 256, 256);

    // printf("==============\n");

    // verify_matmul<0>(16, 512, 512, 512);
    // verify_matmul<1>(16, 512, 512, 512);
    // verify_matmul<2>(16, 512, 512, 512);

    // printf("==============\n");

    // verify_matmul<0>(16, 1024, 1024, 1024);
    // verify_matmul<1>(16, 1024, 1024, 1024);
    // verify_matmul<2>(16, 1024, 1024, 1024);

    // printf("==============\n");

    // verify_matmul<0>(8, 64, 64, 64);
    // verify_matmul<1>(8, 64, 64, 64);
    // verify_matmul<2>(8, 64, 64, 64);

    // printf("==============\n");

    // verify_matmul<0>(8, 128, 128, 128);
    // verify_matmul<1>(8, 128, 128, 128);
    // verify_matmul<2>(8, 128, 128, 128);

    // printf("==============\n");

    // verify_matmul<0>(8, 256, 256, 256);
    // verify_matmul<1>(8, 256, 256, 256);
    // verify_matmul<2>(8, 256, 256, 256);

    // printf("==============\n");

    // verify_matmul<0>(8, 512, 512, 512);
    // verify_matmul<1>(8, 512, 512, 512);
    // verify_matmul<2>(8, 512, 512, 512);

    // printf("==============\n");

    // verify_matmul<0>(8, 1024, 1024, 1024);
    // verify_matmul<1>(8, 1024, 1024, 1024);
    // verify_matmul<2>(8, 1024, 1024, 1024);

    // printf("==============\n");

    // verify_matmul<0>(4, 64, 64, 64);
    // verify_matmul<1>(4, 64, 64, 64);
    // verify_matmul<2>(4, 64, 64, 64);

    // printf("==============\n");

    // verify_matmul<0>(4, 128, 128, 128);
    // verify_matmul<1>(4, 128, 128, 128);
    // verify_matmul<2>(4, 128, 128, 128);

    // printf("==============\n");

    // verify_matmul<0>(4, 256, 256, 256);
    // verify_matmul<1>(4, 256, 256, 256);
    // verify_matmul<2>(4, 256, 256, 256);

    // printf("==============\n");

    // verify_matmul<0>(4, 512, 512, 512);
    // verify_matmul<1>(4, 512, 512, 512);
    // verify_matmul<2>(4, 512, 512, 512);

    // printf("==============\n");

    // verify_matmul<0>(4, 1024, 1024, 1024);
    // verify_matmul<1>(4, 1024, 1024, 1024);
    // verify_matmul<2>(4, 1024, 1024, 1024);

    printf("==============\n");

    verify_matmul<0>(2, 64, 64, 64);
    verify_matmul<1>(2, 64, 64, 64);
    verify_matmul<2>(2, 64, 64, 64);

    printf("==============\n");

    verify_matmul<0>(2, 128, 128, 128);
    verify_matmul<1>(2, 128, 128, 128);
    verify_matmul<2>(2, 128, 128, 128);

    printf("==============\n");

    verify_matmul<0>(2, 256, 256, 256);
    verify_matmul<1>(2, 256, 256, 256);
    verify_matmul<2>(2, 256, 256, 256);

    printf("==============\n");

    verify_matmul<0>(2, 512, 512, 512);
    verify_matmul<1>(2, 512, 512, 512);
    verify_matmul<2>(2, 512, 512, 512);

    printf("==============\n");

    verify_matmul<0>(2, 1024, 1024, 1024);
    verify_matmul<1>(2, 1024, 1024, 1024);
    verify_matmul<2>(2, 1024, 1024, 1024);
