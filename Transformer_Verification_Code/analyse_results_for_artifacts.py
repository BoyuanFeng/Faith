...
for dataset in ["sst", "yelp"]:
    for layer_cnt in range(1,7,1):
        ...
        speed_up[layer_cnt]["tvm"] = np.mean(np.array(times[layer_cnt]["tvm"]))#/np.array(times[layer_cnt]["pytorch"]))
        speed_up[layer_cnt]["faith"] = np.mean(np.array(times[layer_cnt]["faith"]))#/np.array(times[layer_cnt]["faith"]))
    ...
    print("layer_cnt\tpytorch\ttvm\tfaith")
    for layer_cnt in range(1,7,1):
        print("%d\t%f\t%f\t%f"%(layer_cnt, speed_up[layer_cnt]["pytorch"], speed_up[layer_cnt]["tvm"], speed_up[layer_cnt]["faith"]))
