...
home_path = os.getcwd()
log_matmul_pytorch = home_path + '/log_figure10/matmul_pytorch.txt'
log_matmul_cuda = home_path + '/log_figure10/matmul_cuda.txt'
log_matmul_tvm = home_path + '/log_figure10/matmul_tvm.txt'
log_matmul_ansor = home_path + '/log_figure10/matmul_ansor.txt'

os.chdir(home_path + '/../HandTunedKernels/')
os.system('make matmul_artifact1 &> %s'%(log_matmul_cuda))

os.chdir(home_path + '/../Benchmark/')
os.system('python pytorch_benchmark.py --dir model --data yelp --kernel_idx 1 --current_flag 0 &> %s'%(log_matmul_pytorch))
os.system('python pytorch_benchmark.py --dir model --data yelp --kernel_idx 1 --current_flag 2 &> %s'%(log_matmul_tvm))
os.system('python pytorch_benchmark.py --dir model --data yelp --kernel_idx 1 --current_flag 3 &> %s'%(log_matmul_ansor))
...
for line in file_matmul_pytorch.readlines():
    ...
    if line_split[0] == 'batch_size':
        res_matmul_pytorch.append(float(line_split[-1]))
...
print("figure 10 results: ")

print('faith raw:')
print(np.array(res_matmul_cuda))
